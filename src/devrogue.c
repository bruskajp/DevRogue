...
static void genLevel( void ) {
	...
	j = 0;
        while(j < 5){
                ...
                box_pos_x = 1;
                get_random_bytes(&rand, sizeof(rand));
                box_pos_y = (rand%(PLAYFIELD_HEIGHT - 2)) + 1;
                --num_doors_available;
                while(box_pos_x < PLAYFIELD_WIDTH-2) {
                        x = 0;
                        y = 0;

                        while (x < box_size_x && (x + box_pos_x) < PLAYFIELD_WIDTH-2){
                                //printk("AHH: %d < %d &&  %d < %d\n", x, box_size_x, box_pos_x, PLAYFIELD_WIDTH-3);
                                while (y < box_size_y && (y + box_pos_y) < PLAYFIELD_HEIGHT-2) {
                        ...
                        if (box_pos_x > PLAYFIELD_WIDTH-2) { box_pos_x = PLAYFIELD_WIDTH-2; }
                        if (box_pos_y < 1) { box_pos_y = 1; }
			if (box_pos_y > PLAYFIELD_HEIGHT-2) { box_pos_y = PLAYFIELD_HEIGHT-2; }
	...
	} while (gamebuffer[doors[0]+doors[1]*PLAYFIELD_WIDTH-1] == '*');
	...
	while (i < enemyCount){
		do{
			get_random_bytes(&rand, sizeof(rand));
			player_x = rand % (PLAYFIELD_WIDTH - 2) + 1;
			get_random_bytes(&rand, sizeof(rand));
        	        player_y = rand % (PLAYFIELD_HEIGHT-2) + 1;
...
static void rogue_update_state(char action) {
	...
	switch(action) {
		...
		case 'l':
			...
			break;
		case 'r':
			if((playerPos%PLAYFIELD_WIDTH) < PLAYFIELD_WIDTH) {
				if((gamebuffer[playerPos+1] == '.')) {
					playerPos += 1;
				} else if(gamebuffer[playerPos+1] == '^') {
					genLevel();
